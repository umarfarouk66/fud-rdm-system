<?php
/**
 * Controlled Download Endpoint for Chapter Files
 * RDM Information System - Phase 5 Security
 */
...
try {
    ...
    if (!$ver) {
        ...
        die('404 Not Found: Chapter version record not found.');
    }
    ...
    if (!$isOwner && !$isSupervisor && !$memberRole && !$isAdmin) {
        ...
        die('403 Forbidden: You do not have permission to access or download this document.');
    }

    // Resolve file path safely
    $storedFileName = basename($ver['stored_file_name']);
    $fullPath = __DIR__ . '/../storage/chapters/' . $storedFileName;
    ...
    if (!file_exists($fullPath) || !is_file($fullPath)) {
        http_response_code(404);
        die('404 Not Found: The requested chapter document file does not exist on disk.');
    }
    ...
    error_log("Chapter Download Error: " . $e->getMessage());
