<?php
/**
 * Reject Dataset Access Request Controller
...
if (empty($reviewerComment)) {
    $_SESSION['review_errors'] = ['Please provide a reason explaining why the access request is being declined.'];
    header("Location: review.php?id={$requestId}");
    exit;
}
...
try {
    ...
    if (!canManageAccessRequests($projectRole, $systemRole, $datasetOwnerId, $userId)) {
        $pdo->rollBack();
        $_SESSION['access_error'] = 'Access denied: You do not have permission to decline access requests for this dataset.';
        header('Location: index.php');
        exit;
    }
    ...
    log_audit(
        ...
        'access_requests',
        $requestId,
        "Declined access request #{$requestId} for researcher '{$request['requester_first_name']} {$request['requester_last_name']}' (#{$request['requester_id']}) on dataset '{$request['dataset_title']}' (#{$datasetId})"
    );

    // Notify requester of access rejection
    require_once __DIR__ . '/../notifications/notification_helper.php';
    createNotification(
        $pdo,
        (int)$request['requester_id'],
        'access_request_rejected',
        'Dataset Access Request Declined',
        "Your access request for restricted dataset '{$request['dataset_title']}' was reviewed and declined.",
        'access_request',
        $requestId
    );
    ...
    $_SESSION['access_success'] = 'Access request declined. The researcher has been notified with your feedback.';
    header("Location: view.php?id={$requestId}");
    exit;
    ...
    error_log("Access Request Reject Error: " . $e->getMessage());
    $_SESSION['review_errors'] = ['A system error occurred while declining the access request. Please try again.'];
    header("Location: review.php?id={$requestId}");
    exit;
}
